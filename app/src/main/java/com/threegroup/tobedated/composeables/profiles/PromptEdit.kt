...
@Composable
fun PromptEditC(nav: NavHostController, vmCasual: CasualViewModel, questionNumber:Int){
    var question by rememberSaveable { mutableStateOf(true) }
    ...
    ChangePreferenceTopBar(
            ...
            if (question) {
                Column(modifier = Modifier.fillMaxSize()) {
                    ...
                    Column(
                        ...
                        questionsToUse.forEach { quest ->
                            if (quest == currentUser.casualAdditions.promptQ1 || quest == currentUser.casualAdditions.promptQ2 || quest == currentUser.casualAdditions.promptQ3) {
                                PlainTextButton(
                                    question = quest,
                                    onClick = { },
                                    enabled = false
                                ...
                                PlainTextButton(
                                    ...
                                    onClick = {
                                        ...
                                        question = false
                                        when (questionNumber) {
                                            1 -> currentUser.casualAdditions.promptQ1 = quest
                                            2 -> currentUser.casualAdditions.promptQ2 = quest
                                            3 -> currentUser.casualAdditions.promptQ3 = quest
                                        }
                                    }
                                )
                                Spacer(modifier = Modifier.height(6.dp))
        ...
        save = {
            Button(
                ...
                onClick = {
                    when (questionNumber) {
                        1 -> currentUser.casualAdditions.promptA1 = prompt
                        2 -> currentUser.casualAdditions.promptA2 = prompt
                        3 -> currentUser.casualAdditions.promptA3 = prompt
                    }
                    nav.popBackStack()
